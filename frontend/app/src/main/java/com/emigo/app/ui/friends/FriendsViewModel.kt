...
class FriendsViewModel(
    ...
    // Same pattern WidgetSettingsViewModel already uses for its own Gold-gated action — a plain
    // client-side flag the UI reads to decide whether tapping "Restore streak" should call the
     ...
     * per-row busy-state shape, scoped to this one action instead of shared with it, since a
     * restore and an accept/decline are never in flight for the same row at the same time but
     * conflating their busy-sets would still read as a bug if they ever were. */
    var restoringStreakFriendshipIds by mutableStateOf<Set<String>>(emptySet())
        private set
    ...
    /** Only ever called once the caller has already confirmed [isGoldMember] — see
     * FriendsScreen's own restore-pill onClick, which redirects to the Gold screen instead of
     * calling this at all when it's false. The server independently re-checks Gold status itself
     * regardless (FriendService.restoreStreak), so this being called for a non-Gold user would
     * just surface as a normal [errorMessage] rather than silently succeeding. */
    fun restoreStreak(friendshipId: String) {
